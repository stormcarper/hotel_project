...
``src/hotel_project/conf/local.py``. You can base this file on the
...
.. code-block:: bash
    ...
    python src/manage.py test hotel_project
...
* ``DB_NAME``: name of the database for the project. Defaults to ``hotel_project``.
* ``DB_USER``: username to connect to the database with. Defaults to ``hotel_project``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``hotel_project``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       docker-compose up -d
       Starting hotel_project_db_1 ... done
       Starting hotel_project_web_1 ... done
   ...
   .. code-block:: bash
       ...
       docker logs -f hotel_project_web_1
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       docker exec -it hotel_project_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       docker exec -it hotel_project_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/maykinmedia/hotel_project/
...
environment variables. See ``src/hotel_project/conf/docker.py`` for
all settings.

.. code-block:: bash

    docker build -t hotel_project
    docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    docker exec -it hotel_project /app/src/manage.py createsuperuser
